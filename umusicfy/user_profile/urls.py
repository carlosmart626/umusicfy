...
urlpatterns = [
    ...
    url(r'^(?P<username>[\w-]+)/playlist/$', login_required(PlayListListView.as_view()), name='user_playlists'),
    ...
    url(r'^(?P<username>[\w-]+)/(?P<playlist_slug>[\w-]+)$', login_required(PlaylistDetailView.as_view()), name='user_playlist'),
    url(r'^add-song/(?P<playlist_id>[\w-]+)/(?P<song_id>[\w-]+)$', login_required(AddToPlaylistView.as_view()), name='add_song_playlist'),
